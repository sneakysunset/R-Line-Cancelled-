...
public class Player_Jump : MonoBehaviour
{
    ...
    void Falling()
    {
        ...
        if (rb.velocity.y < 0) rb.velocity += (Vector2)transform.up * Time.deltaTime * Physics2D.gravity.y * (fallMult - 1);
        else if (rb.velocity.y > 0 && !player.jumpingInput) rb.velocity += (Vector2)transform.up * Time.deltaTime * Physics2D.gravity.y * (lowJumpMult - 1);
    }
    ...
    void ArtificialGravity()
    {
        ...
        if(player.groundCheck && player.moving)
        {
            ...
            RaycastHit2D hit = Physics2D.Raycast(rayCastPoint.position, (-transform.up + .5f * transform.right).normalized, 1f);
